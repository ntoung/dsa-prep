...
`dsa-prep:activity-log` is the one raw event log; everything about *when* review happened (streaks, the daily-activity heatmap, today's progress toward the goal) is derived from it at read time rather than persisted as its own counter, so those views can never drift out of sync with the log — see `dailyCountsFromLog` in `src/lib/streak.ts`.
...
### `dsa-prep:activity-log` — owned by `useReviewState.ts` — the raw log

`ActivityLogEntry[]`, one entry per review action, unbounded (each entry is tiny, and this is now the sole source for daily/streak history, so it's never trimmed):
...
`dailyActivity` (`Record<YYYY-MM-DD, count>`, for the streak and the calendar heatmap) and `todayCount` (today's progress toward the daily goal) are *not* separate localStorage keys — they're computed from this log on every render via `useMemo` in `useReviewState.ts`.
